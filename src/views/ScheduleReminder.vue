<template>
  <div class="wrapper">
    ...
    <BroadcastConfirmationModal
      ...
      title="Konfirmasi Broadcast Berulang"
      confirm-label="Konfirmasi Jadwalkan"
      :items="confirmationItems"
      :loading="loading"
      notice="Broadcast akan berjalan otomatis sesuai rentang tanggal dan interval ini."
...
const recurrenceLabel = computed(() => {
  const labels = { daily: "hari", weekly: "minggu", monthly: "bulan" };
  return `Setiap ${form.value.interval || 1} ${labels[form.value.recurrence] || form.value.recurrence}`;
});
...
const confirmationItems = computed(() => [
  ...
  { label: "Mulai", value: formatLocalTime(convertToServerTime(form.value.startDate)) },
  { label: "Selesai", value: formatLocalTime(convertToServerTime(form.value.endDate)) },
  { label: "Pengulangan", value: recurrenceLabel.value },
