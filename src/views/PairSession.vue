    ...
    <section class="start card">
      ...
      <div class="row">
        ...
        <div class="row-btns">
          ...
          >
            {{ loading ? "Menunggu QR..." : "Mulai" }}
          </button>
          <button v-if="controller" class="btn outline" @click="stopPairing">
            Hentikan
          </button>
        </div>
      </div>
      <p class="hint">
        Scan QR dari WhatsApp di ponsel Anda. QR mungkin diperbarui beberapa kali.
      </p>
      <p v-if="selectedStatus === 'open'" class="hint success">
        Device sudah terhubung. Hapus sesi jika ingin Pairing ulang.
      </p>
      ...
      <div v-if="qr" class="qr-container">
        ...
        <p class="qr-instructions">
          Buka WhatsApp di ponsel Anda → Ketuk titik tiga → Perangkat Tertaut → Tautkan
          Perangkat → Scan QR code di atas
        </p>
      </div>
    ...
    <section class="sessions card" v-if="showSessions">
      ...
      <ul>
        <li v-for="(s, idx) in sessions" :key="s.sessionId || idx" class="session-item">
          <span
            >{{ s.device && s.device.name ? s.device.name : "-" }} —
            {{ s.device && s.device.phone ? s.device.phone : "-" }} —
            {{ s.device && s.device.status ? s.device.status : s.status || "-" }}</span
          >
          <span v-if="s.sessionId" class="session-id">
            — sessionId: {{ s.sessionId }}</span
          >
          <button
            v-if="(s.device?.status || s.status) === 'open' && s.sessionId"
            class="btn"
            @click="onSendTest(s.sessionId)"
          >
            Kirim pesan tes
          </button>
        </li>
